...
# My Parameters
tf.app.flags.DEFINE_string("eeg_train_data", "../temazepam_list/sctrain_list.txt", "Point to directory of input data")

tf.app.flags.DEFINE_string("eeg_eval_data", "../temazepam_list/eval_list.txt", "Point to directory of input data")

tf.app.flags.DEFINE_string("eeg_test_data", "../temazepam_list/train_list.txt", "Point to directory of input data")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def evaluate(gen_wrapper):
            ...
            labels = np.zeros([N, config.epoch_seq_len])
            ...
            for data_fold in range(config.num_fold_testing_data):
                ...
                yhat_, score_, output_loss_, total_loss_, feature, labels_ = _evaluate(gen_wrapper.gen)
                ...
                labels[count: count + len(gen_wrapper.gen.data_index)] = labels_
            ...
            return test_acc, yhat, score, output_loss, total_loss, all_features, labels
        ...
        def _evaluate(gen):
            ...
            labels = np.zeros([len(gen.data_index), config.epoch_seq_len])
            ...
            while test_step < num_batch_per_epoch:
                ...
                labels[(test_step - 1) * factor * config.batch_size: test_step * factor * config.batch_size] = label_batch_
                test_step += 1
            ...
            if (gen.pointer < len(gen.data_index)):
                ...
                labels[(test_step - 1) * factor * config.batch_size: len(gen.data_index)] = label_batch_
                output_loss += output_loss_
                total_loss += total_loss_
            ...
            return yhat, score, output_loss, total_loss, feature, labels


        # evaluation on test data
        start_time = time.time()
        test_acc, test_yhat, test_score, test_output_loss, test_total_loss, all_features, labels = evaluate(gen_wrapper=test_gen_wrapper)
        end_time = time.time()
        with open(os.path.join(out_dir, "test_time.txt"), "a") as text_file:
            text_file.write("{:g}\n".format((end_time - start_time)))

        hdf5storage.savemat(os.path.join(out_path, "temazepam_ret.mat"),
                            {'yhat': test_yhat,
                             'acc': test_acc,
                             'score': test_score,
        ...
        out_file_path_Nd = os.path.join(out_dir, 'st_temazepam_v2.pkl')
